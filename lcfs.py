...
def output(file_name):
    ...
    with open('output.txt', 'a') as f:
        ...
        f.write('Sredni czas oczekiwania: ' +
                str(round(calculate_average_waiting_time(list(reversed(seq)) for seq in parse(file_name)), 2)) + '\n')
        f.write('Sredni czas cyklu przetwarzania: ' +
                str(round(calculate_average_turnaround_time(list(reversed(seq)) for seq in parse(file_name)), 2)) + '\n')
        f.write('\n')
